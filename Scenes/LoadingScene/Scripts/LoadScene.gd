...
func load_screen_to_scene(target: String, parameters: Dictionary = {}) -> void:
	var loading_screen = preload("res://LoadingScene/Load.tscn").instantiate() # Godot handles loading the loading screen upon game startup. We'll instance a new one here.
